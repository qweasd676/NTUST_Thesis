...
if __name__ == "__main__":
    ...
    ret,img_test = cv2.threshold(img.copy(),100,255,cv2.THRESH_BINARY)
    print(type(img_test))
    cv2.imshow("img1",img_test)
    cv2.waitKey(0)
    size = img.shape
    ...
    #Dirac()
    level = levelset_()
    level.initializePhi(~img_test, rect)
    level.EVolution()
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    cv2.imshow("img", showimg)
    cv2.imwrite('level_set_img.jpg',showimg)
    cv2.waitKey(0)